...
This mode is for factual inspection and development diagnostics. It is not the product server topology.
...
The console prints a bounded summary with source coverage plus Observation and Evidence previews. The full JSON is written under `.argus/probes/<collection_id>.json` unless `--output` is supplied.
...
1. `result.status` and `result.errors`;
2. `result.coverage` to see which factual adapters actually ran;
3. `result.observations[*].source_kind`, `url`, `data`, `provenance`, and `quality`;
4. `result.evidence[*].source.url` and `text` to confirm the observation is backed by the fetched source;
5. `collection.checkpoint.discovery_queries` / provider metadata when discovery was used;
6. source health and runtime metrics for blocked, degraded, retried or escalated paths.
