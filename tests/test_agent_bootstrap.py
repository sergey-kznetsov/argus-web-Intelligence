...
@pytest.mark.asyncio
async def test_browser_use_agent_is_optional_last_resort_with_bounded_health_contract(
    tmp_path: Path,
):
    settings = Settings(
    ...
    assert isinstance(adapter.agent, BrowserUseAgent)

    health = await adapter.health()
    agent = health["agent_execution"]
    assert agent["backend"] == "browser-use"
    ...
    assert agent["max_actions"] == 40
    assert agent["max_visited_urls"] == 20
