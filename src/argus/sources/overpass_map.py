...
class OverpassSourceAdapter:
    ...
    def __init__(
        ...
        *,
        skip_ungeocoded_discovery: bool = False,
    ) -> None:
        self.provider = provider
        self.snapshots = snapshots
        self.geocoder = geocoder
        self.skip_ungeocoded_discovery = bool(skip_ungeocoded_discovery)

    async def discover(self, request: CollectionRequest) -> list[SourceTask]:
        # Auto-enabled server Overpass is useful only when the caller already supplied a
        # point (Geo Analyzer/Kraken do this for radius analyses) or a geocoder exists.
        # Explicitly configured adapters retain their historical behavior and surface a
        # GEOCODING_NOT_CONFIGURED error instead of silently changing the source contract.
        if (
            self.skip_ungeocoded_discovery
            and request.territory.point is None
            and self.geocoder is None
        ):
            return []
