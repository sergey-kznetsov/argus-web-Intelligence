...
class SQLiteRepository:
    ...
    async def prune_recipe_versions(
        ...
        *,
        ...
        return await self._run(self._prune_recipe_versions_sync, domain, goal, keep)

    def _prune_recipe_versions_sync(self, domain: str, goal: str, keep_versions: int) -> int:
        with self._connect() as conn:
            cursor = conn.execute(
                """
                ...
                """,
                (domain, goal, domain, goal, keep_versions),
            )
